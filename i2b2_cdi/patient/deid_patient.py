...
class DeidPatient:
    ...
    def __init__(self):
        ...
        self.deid_header = ['PatientID', 'VitalStatusCD', 'BirthDate', 'DeathDate', 'SexCD', 'AgeInYears',
                            'LanguageCD', 'RaceCD', 'MaritalStatusCD', 'ReligionCD', 'ZipCD', 'StateCityZipPath', 'IncomeCD']
        self.error_file_header = ['PatientID', 'VitalStatusCD', 'BirthDate', 'DeathDate', 'SexCD', 'AgeInYears', 'LanguageCD',
                                  'RaceCD', 'MaritalStatusCD', 'ReligionCD', 'ZipCD', 'StateCityZipPath', 'IncomeCD', 'ValidationError', 'ErrorRowNumber']

    def deidentify_patient(self, patient_map, csv_file_path, input_csv_delimiter, deid_file_path, output_deid_delimiter, error_file_path):
        ...
        logger.info('De-identifing patient file : ' + csv_file_path)
        try:
            # Write file header
            ...
            with open(csv_file_path, mode='r') as csv_file:
                ...
                with alive_bar(max_line, bar='smooth') as bar:
                    for row in csv_reader:
                        ...
                        if row['BirthDate'] and not self.is_valid_date_format(row['BirthDate']):
                            _validation_error.append("Invalid birth date format")
                        if row['DeathDate'] and not self.is_valid_date_format(row['DeathDate']):
                        ...
                        if patient_num is None:
                            _validation_error.append("Patient mapping not found")
                        else:
...
def do_deidentify(csv_file_path):
    ...
    if os.path.exists(csv_file_path):
        ...
        D.deidentify_patient(patient_map, csv_file_path, input_csv_delimiter,
